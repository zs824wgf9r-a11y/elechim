#!/usr/bin/env python3
"""Prova: l'estrazione di Honcho regge l'italiano?

Usa il prompt vero del deriver (plastic-labs/honcho, revisione citata nel
rapporto) su qwen3:8b del fisso, con lock GPU e blocco sospensione.
Non installa Honcho, Postgres ne' pgvector.

I dati in archivio/ vengono toccati solo dalla macchina: questo script legge
i messaggi, chiama il modello e scrive le osservazioni in stato/prova-estrazione/.
Il rapporto contiene solo numeri.
"""
...
# Prompt esatto minimal_deriver_prompt dal sorgente Honcho (src/deriver/prompts.py,
# revisione sopra). L'unica aggiunta e' l'ultima riga: istruzione di formato
# per poter contare le osservazioni in modo meccanico. Non cambia il compito
# semantico.
...
# Criterio di lingua: parole funzione. Una osservazione e' italiana se ha
# almeno 2 parole funzione italiane e il rapporto it/en supera 1.5;
# inglese se almeno 2 inglesi e en/it supera 1.5; altrimenti mista/indeterminata.
PAROLE_ITALIANE = {
    "il", "la", "lo", "i", "le", "gli",
...
def estrai_osservazioni(testo: str) -> list[str]:
    """Estrae righe che iniziano con '- ' come osservazioni."""
    out = []
...
def formatta_messaggi(messaggi: list[dict]) -> str:
    ...
    for m in messaggi:
        ...
        if not contenuto:
            continue
        # Il target peer e' il proprietario; i suoi messaggi hanno ruolo 'user'.
...
def scrivi_osservazioni(nome: str, messaggi: list[dict], osservazioni: list[str]) -> None:
...
def main() -> int:
    ...
    with energia.blocco("prova-estrazione"), energia.riserva_gpu("prova-estrazione"):
        risultati = []
        for db in db_files:
            nome = db.stem
            print(f"\n--- {nome} ---")
            ...
            prompt = PROMPT_TEMPLATE.format(peer_id=PEER_ID, messages=testo_messaggi)
            ...
            scrivi_osservazioni(nome, messaggi, osservazioni)
